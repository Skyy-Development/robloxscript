...
local metadata = urlLoad("https://raw.githubusercontent.com/quaxfn/robloxscript/main/metadata.lua")
...
local SaveManager = {} do
    ...
    function SaveManager:Save(name)
        local fullPath = 'SkyBlock_autoplayer/configs/' .. name .. '.json'
    ...
    function SaveManager:Load(name)
        local file = 'SkyBlock_autoplayer/configs/' .. name .. '.json'
    ...
    function SaveManager.Refresh()
        local list = listfiles('SkyBlock_autoplayer/configs')
...
local Window = UI:CreateWindow({
    Title = string.format('SkyBlockAutoPlayer - version %s | updated: %s', metadata.version, metadata.updated),
    AutoShow = true,
...
if type(readfile) == 'function' and type(writefile) == 'function' and type(makefolder) == 'function' and type(isfolder) == 'function' then
    makefolder('SkyBlock_autoplayer')
    makefolder('SkyBlock_autoplayer\\configs')
...
do
    ...
    themeManager:SetFolder('skyblockautoplayer')
...
UI:Notify(string.format('Loaded SkyblockAutoPlayer in %.4f second(s)!', tick() - start), 3)
